...
/* abcd 与 invader 图标像素点阵映射,efg 与 fighter
 * 图标像素点阵映射,h 与炮弹图标像素点阵映射。*/
...
void HariMain(void)
{
    ...
    /* 新建缓冲区为winbuf(336*261)标题为invader无透明色窗口,
     * 在新建窗口中绘制矩形[(6,27),(329,254)];分配定时器及其超时数据128。*/
...
next_group:
    ...
    for (;;) {
        ...
        /* fighter移动,
         * keyflag[0],left; keyflag[1],right; keyflag[2],发射 */
        ...
        if (ly > 0) {
            ...
            if (ix < lx && lx < ix + 25 && iy <= ly && ly < iy + invline) {
                ...
                if (*p != ' ') {
                    ...
                    /* invader被全部消灭则跳转 next_group处进行下一组invader */
...
/* putstr,
 * 将s所指字符串以色号col在win所管理窗口(x,y)处开始显示。*/
void putstr(int win, char *winbuf, int x, int y, int col, unsigned char *s)
{
    int c, x0, i;
    char *p, *q, t[2];

...
; BMP 解析程序,I.Tak不沿用已有汇编格式而选择nasm汇编格式编写 BMP 解析程序,
...
; BMP 文件头部信息
struc BMP
    ;FILE HEADER,BMP文件头部信息
    .fType: resw 1  ;BM
    .fSize: resd 1  ;whole file size
            resd 1  ;reserved
    .fOffBits: resd 1 ;offset from file top to image(图像数据在图片文件中的偏移)

    ;INFO HEADER,BMP图像头部信息
...
; _info_BMP
; 解析 BMP 图片。
_info_BMP: ; int info_BMP(struct DLL_STRPICENV *env, int *info, int size, char *fp);
    push ebx
    push ebp
    push esi
    push edi
